...
define("ERR_UPSTREAM","Upstream didn't respond, please refresh the page and try again!");
function getId($id){
...
function main(){
	try {
		...
		if(!$url)  throw new StalkErr(ERR_UPSTREAM);
		echo json_encode(array("title" => htmlspecialchars($id->name,ENT_QUOTES)." | ".ucfirst(htmlspecialchars($id->gender,ENT_QUOTES)),
			 "data"=>"data:image/jpg;base64,".base64_encode(file_get_contents(preg_replace("/[0-9]{3}x[0-9]{3}/i","720x720",$url)))));
	    }
	catch(Exception $e)
	 {
		echo json_encode(array("error" => $e->getMessage()."\n\nThis was caught at ".$e->getLine()));
	 } }
